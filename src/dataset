...
class GraphDataset(PyGDataset):
    ...
    def get(self, idx):
        ...
        try:
            ...
            # Feature slicing based on new format
            h_res_type = original_x[:, :20]
            # h_is_bs    = original_x[:, 20:21] # Not used in clean feature
            # h_disp     = original_x[:, 21:23] # Not used in clean feature
            ...
            # Remove redundant columns for clean set
            # 20(AA) + 1(is_bs) + 2(disp) + 1(elem) + 3(rel_pos) + 1(dist) + 7(rdkit)
            # Reconstruct explicitly what the model needs:
