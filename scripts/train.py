...
def train_epoch(model, loader, criterion_act, criterion_bind,
                optimizer, scaler, device, accum_steps, lambda_act, w_fn):
    """Run one full training epoch with gradient accumulation and AMP.

    Args:
        model: GPCRact_Model instance.
        loader: Training DataLoader.
        criterion_act: CrossEntropyLoss (reduction='none') for Stage-2.
        criterion_bind: BCEWithLogitsLoss for Stage-1.
        optimizer: AdamW optimizer.
        scaler: torch.amp.GradScaler instance (created once in main()).
        device: torch.device.
        accum_steps: Gradient accumulation steps.
        lambda_act: Weight for Stage-2 activity loss.
        w_fn: Callable returning per-sample class weights for Stage-2, or None.

    Returns:
        Tuple of (mean_total_loss, mean_act_loss, mean_bind_loss).
    """
    model.train()
    # Derive is_cuda from the passed device; never rely on module-level state.
    ...
    for i, data in enumerate(tqdm(loader, desc="Training", leave=False)):
        # collate_fn returns None when the entire mini-batch is invalid.
        ...
        # Note: .squeeze(-1) is unsafe when labels are already [B]: squeezing a [B=1]
        # tensor yields a 0-dim scalar and breaks mask indexing on B=1 mini-batches.
        ...
        with torch.autocast(device_type=device.type, enabled=is_cuda):
            # Model returns (binding_logit [B,1], activity_type_logit [B,2]).
            ...
            if valid_bind_mask.any():
                loss_bind = criterion_bind(
                    logits_bind[valid_bind_mask],
                    binding_labels[valid_bind_mask],
                )

            valid_act_mask = activity_labels != -1
            if valid_act_mask.any():
                loss_act = criterion_act(
                    logits_act[valid_act_mask],
                    activity_labels[valid_act_mask],
                )
                if w_fn is not None:
            ...
            # Stage-1 binding loss keeps unit weight; class imbalance is handled
            # via pos_weight in the BCE criterion (FN penalty = 1.5).
            loss = (lambda_act * loss_act + loss_bind) / accum_steps
    ...
    return (
        ...
        total_act_loss  / valid_batches,
        total_bind_loss / valid_batches,
    )

def evaluate(model, loader, device, return_df=False):
    """Evaluate model on a DataLoader and return balanced accuracy scores.

    Args:
        model: GPCRact_Model instance.
        loader: Validation or test DataLoader.
        device: torch.device.
        return_df: If True, also return a per-sample results DataFrame.

    Returns:
        (act_bacc, bind_bacc) or (act_bacc, bind_bacc, results_df).
    """
    model.eval()
    ...
    # Derive locally; do not rely on any module-level is_cuda.
    is_cuda = device.type == "cuda"
    ...
    with torch.no_grad():
        for data in tqdm(loader, desc="Evaluating", leave=False):
            ...
            with torch.autocast(device_type=device.type, enabled=is_cuda):
                binding_logit, logits_act = model(protein_batch, ligand_batch)
                logits_bind = binding_logit.reshape(-1)
...
def main(args):
    ...
    # GradScaler is created ONCE here and passed into train_epoch every epoch
    # so its internal loss-scale state accumulates correctly across epochs.
    is_cuda = device.type == "cuda"
    ...
    train_csv_path = (
        Path(args.train_csv) if args.train_csv
        else Path(args.data_dir) / "scaffold_train.csv"
    )
    val_csv_path = (
        Path(args.val_csv) if args.val_csv
        else Path(args.data_dir) / "scaffold_val.csv"
    )
    ...
    # Separate Generator objects: sharing a single Generator between two
    # DataLoaders causes the val loader to see a different RNG state depending
    # on how many batches the train loader has consumed.
    ...
    class_weights  = len(act_labels_np) / (len(class_counts) * class_counts)
    ...
    for epoch in range(1, args.epochs + 1):
        t_loss, a_loss, b_loss = train_epoch(
            model, train_loader, criterion_act, criterion_bind,
            optimizer, scaler, device,
            args.accum_steps, args.lambda_act, get_class_weights,
        )
        val_bacc, val_bind_bacc = evaluate(model, val_loader, device)

        print(
            f"Epoch {epoch:03d} | Train Loss: {t_loss:.4f} "
            f"| Val Act BACC: {val_bacc:.4f} "
            f"| Val Bind BACC: {val_bind_bacc:.4f}"
        )

        # Pass optimizer/scaler/epoch/args so the checkpoint is fully resumable.
        early_stopping(
            val_bacc, model,
            optimizer=optimizer,
            scaler=scaler,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_dir", type=str, default="../data/splits",
        help="Default location for scaffold_train.csv and scaffold_val.csv "
             "(used when --train_csv / --val_csv are not provided).",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--lambda_act", type=float, default=1.0,
        help="Weight for Stage-2 activity loss (Table S5: 1.0).",
    )
    parser.add_argument(
        "--binding_fn_penalty", type=float, default=1.5,
